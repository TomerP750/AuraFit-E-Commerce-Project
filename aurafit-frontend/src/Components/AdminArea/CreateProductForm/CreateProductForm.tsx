...
export function CreateProductForm(): JSX.Element {
    ...
    useEffect(() => {
        adminService.allCategories().then(setCategories).catch(err => toast.error(err));
        adminService.allSubCategories().then(setSubCategories).catch(err => toast.error(err));
    }, []);
    ...
    const onSubmit = (data: FormValues): void => {
        ...
        // instantiate DTO without variants
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 bg-white rounded shadow space-y-6">
            {/* Description */}
            <div>
